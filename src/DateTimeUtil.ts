export class DateTimeUtil {
    ...
    static format(inputDate: string | Date, format: string): string {
        ...
        const jpnEra = eraParts.find(p => p.type === 'era')?.value || '';
        const jpnEraYear = eraParts.find(p => p.type === 'year')?.value || '';
    ...
    static formatFromMinuteTime(timeOrMinute: string | number, format: string, isExcludeZero: boolean) {
        let minute = 0;
        let hour = 0;
